...
const common = {
  ...
  ...(process.env.CLOUDFLARE_R2_JURISDICTION
    ? { jurisdiction: process.env.CLOUDFLARE_R2_JURISDICTION }
    : {}),
};
...
const admin =
  process.env.CLOUDFLARE_R2_ADMIN_ACCESS_KEY_ID &&
  process.env.CLOUDFLARE_R2_ADMIN_SECRET_ACCESS_KEY
    ? createR2ArtifactAdminStore({
        ...common,
...
const scope = { projectId: 'agentos-smoke', runId: randomUUID(), stepId: 'r2' };
...
try {
  ...
  if (stored && admin) await admin.delete(stored.key);
}
