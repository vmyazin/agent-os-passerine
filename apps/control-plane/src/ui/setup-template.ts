...
// first live end-to-end run. Placeholders: project name, repository, branch.
export const SETUP_CONFIG_TEMPLATE = `version: 1
project:
  name: my-project
...
agents:
  specifier:
    ...
    prompt: |
      ...
      For each requirement, add one criterion and one acceptance test. You have no write tools and never create files: each test is a string carried inside the dod artifact below, as one entry of acceptanceTests whose path is test/acceptance/<criterion id>.test.mjs and whose content is the whole file. It must fail if that requirement is unmet, including the negative cases the requirement names (mutation, missing ids, identity). Do not test only the easy half of a copy. Exactly one test per criterion and no others. Trusted code writes these files into the repository later and runs them from the repository root with node --test, so import the implementation by its repository-relative path and use only node:test and node:assert -- no test framework and no new dependencies. For example, src/example.mjs is imported from test/acceptance/<id>.test.mjs as ../../src/example.mjs.
      ...
      keep it verbatim. Copy that object exactly as the tool returned it, every field,
      unedited. It has ten fields, among them key, digest, sizeBytes, createdAt and
      expiresAt; if the object you send back has fewer, you rebuilt it instead of
      copying it and the run is rejected.
  ...
  planner:
    ...
    prompt: |
      ...
      Copy it exactly as returned, every field, unedited — rebuilding it from memory
      drops fields the run requires and is rejected.
  ...
  implementer:
    ...
    prompt: |
      ...
      the diff minimal and complete. Do not add, modify, or delete files under test/acceptance/; trusted code overlays the approved acceptance tests. Materialize the repository files into a working
      ...
      structuredContent.metadata is the artifact reference. Copy each one exactly as
      returned, every field, unedited — rebuilding it from memory drops fields the run
      requires and is rejected.
